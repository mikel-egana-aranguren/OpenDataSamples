...
/////////////////////////////////////////////////////////////////////////////////////////
		...
		@GwtIncompatible("_toMap method uses refection")
		public <K> Map<K,T> toMapIndexingByField(final String oidFieldName) {
			return _toMap(_theCol,oidFieldName);
		}
		public <K> Map<K,T> toMap(final MapEntryKeyExtractor<K,T> keyExtractor) {
...
/////////////////////////////////////////////////////////////////////////////////////////
	    ...
	    public T pickOneAndOnlyElement(final String errMsg,final Object... params) {
			if (CollectionUtils.isNullOrEmpty(_theCol)) throw new IllegalStateException("The collection is empty");
			if (_theCol.size() > 1) throw new IllegalStateException(Throwables.message(errMsg,params));
...
///////////////////////////////////////////////////////////////////////////////
    /**
     * Extracts a {@link Map} value's key
     * @param <K>
     * @param <V>
     */
    public static interface MapEntryKeyExtractor<K,V> {
    ...
    static <K,V> Map<K,V> _toMap(final Collection<V> theCollection,
    							 final MapEntryKeyExtractor<K,V> keyExtractor) {
        Map<K,V> outMap = null;
     ...
     * @return the {@link Map}
     */
    @GwtIncompatible("GWT does NOT supports TypeRef")
    static <K,V> Map<K,V> _toMap(final Collection<V> theCollection,
    							 final String oidFieldName) {
        String theOidFieldName = oidFieldName;
        if (theOidFieldName == null) theOidFieldName = "key";   // Ñapa para un bug...
        Map<K,V> outMap = null;
        if (theCollection != null) {
            outMap = new HashMap<K,V>(theCollection.size());
            // iterate over array elements and put them in the Map           
