...
//		mdConfig.getDataType().getName();
		...
		if (qry.getClause() instanceof BooleanQueryClause) {
			...
			outEncodedQry = Strings.of("{}.{}.{}({}){}")
								   .customizeWith(qry.getClause().getFieldId(),				// [1] field id
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public static <Q extends QueryClause> QualifiedQueryClause<Q> decode(final String encodedQry,
																		 final ModelObjectTypeMetaData metaData) {
		...
		if (m.find()) {
			...
			//														 : null;
			FieldMetaData mdConfig = metaData.getFieldMetaDataFor(fieldId.asMetaDataId());
			if (mdConfig == null) throw new IllegalStateException(Throwables.message("There's NO field with name {} at {} model object metadata; available fields are {}",
																					 fieldId.asMetaDataId(),metaData.getType(),CollectionUtils.of(metaData.getFieldsMetaDataIds()).toStringCommaSeparated()));
			FieldMetaData fieldMetaData = metaData.getFieldMetaDataFor(fieldId.asMetaDataId());
			...
			case beEqualTo:
				if (fieldMetaData.isCollectionField()) {
					...
					FieldMetaDataForCollection colFieldMetaData = metaData.getFieldMetaDataFor(fieldId.asMetaDataId());
					...
					FieldMetaDataForPolymorphicType polyFieldMetaData = metaData.getFieldMetaDataFor(fieldId.asMetaDataId());
					Class<?> polyDataType = polyFieldMetaData.getFieldDataTypeMap().get(metaData.getType());
					clause = (Q)EqualsQueryClause.forField(fieldId)
			...
			case beWithin:
				...
				clause = (Q)ContainedInQueryClause.forField(fieldId)
												  .within(QueryClauseSerializerUtils.spectrumArrayFromString(value,fieldMetaDataForCollection.getComponentsType()));
				break;
			case haveData:
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	private enum CONDITION 
	  implements EnumExtended<CONDITION> {
		...
		private static EnumExtendedWrapper<CONDITION> _enums = EnumExtendedWrapper.create(CONDITION.class);
