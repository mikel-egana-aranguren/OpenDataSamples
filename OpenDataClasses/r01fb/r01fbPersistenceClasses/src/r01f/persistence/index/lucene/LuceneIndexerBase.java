...
/////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 * @param luceneIndex
	 * @param idxDocFieldCfgCustomizer
	 ...
	 */
	public LuceneIndexerBase(final Class<P> modelObjectType,
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public void index(final UserContext userContext,
		...
		Document doc = _createLuceneDocumentFor(userContext,
												modelObject,
	...
	public void updateIndex(final UserContext userContext,
		...
		IndexableFieldID docIdFieldId = modelObject.getModelObjectMetaData().getDocumentIDFieldMetaData().getIndexableFieldId();
		_luceneIndex.reIndex(new Term(docIdFieldId.asString(),			
								  	  _luceneIdFieldValueFrom(modelObject)),
							 doc);
	}
	@Override
	...
	private Document _createLuceneDocumentFor(final UserContext userContext,
											  final P modelObj,
											  final PersistenceRequestedOperation reqOp) {
		// [1] - Extract the field values from the model object
		IndexDocumentFieldValueSet fieldsValues = _extractIndexableFields(userContext,
																	  	  modelObj,
																	  	  reqOp);
		// [2] - Add the id field (every document has an id field)
		FieldMetaData docIdField = modelObj.getModelObjectMetaData().getDocumentIDFieldMetaData();
		...
		if (fieldsValues.hasData()) {
			log.debug("Creating a Lucene document for {} with fields: {}",fieldsValues.getFieldIdsAsString(),_fieldsConfigSet.getClass().getSimpleName());
			...
			for (IndexDocumentFieldValue<?> fieldValue : fieldsValues) {
				...
				try {
					log.debug("- Document field {}.{} of type {}",
							  _fieldsConfigSet.getClass().getSimpleName(),fieldValue.getFieldId(),
							  fieldValue.getValue() != null ? fieldValue.getValue().getClass() : "[NO VALUE]");
					
					if (fieldValue.getFieldId().is(docIdField.getIndexableFieldId())) {
						// The lucene DOCID field
						_createLuceneIdField(docFactory,
								 			 (IndexDocumentFieldValue<String>)fieldValue);
					} else {
						// An ordinary field
...
/////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 * @return
	 */
	private String _luceneIdFieldValueFrom(final P modelObj) {
		String outIdValue = null;
	...
	private <T> void _createField(final LuceneDocumentFactory fieldsFactory,
		...
		if (fieldValue instanceof OID) {
			...
			if (!indexableFieldValue.dependsOnDynamicDimension()
			 && _fieldsConfigSet.getConfigFor(IndexDocumentFieldID.fieldIdOf(fieldId)).getType() == IndexDocumentStandardFieldType.Text) {
				// Full-text field
			...
			for (Map.Entry<?,?> me : fieldValues.entrySet()) {
				IndexDocumentFieldID luceneFieldId = IndexDocumentFieldID.fieldIdOf(indexableFieldValue.getFieldId(),
																   					me.getKey());		// the point
		...
		_process(indexableField,
				 new FieldFactory<String>() {
							...
							public void create(final IndexDocumentFieldID luceneFieldId,
											   final String value) {
								_createLuceneIdField(fieldsFactory,
													 luceneFieldId,
	...
	private static void _createSummaryField(final LuceneDocumentFactory fieldsFactory,
		...
		if (summary != null) {
			if (summary.isLangDependent()) {
				...
				log.debug("\t-Lang Dependent Summary field: {} = {}",luceneFieldId,Strings.customized("--Lang dependant summary in {} languages",CollectionUtils.of(langDepSummary.getAvailableLanguages())
				...
				log.debug("\t-Lang Independent Summary field: {} = {}",luceneFieldId,langIndSummary.asString());
	...
	private static <T> void _process(final IndexDocumentFieldValue<T> indexableField,
		...
		if (indexableField.dependsOnDynamicDimension()) {
			...
			for (Object point : indexableField.getDynamicDimensionPoints()) {
				IndexDocumentFieldID luceneFieldId = IndexDocumentFieldID.fieldIdOf(indexableField.getFieldId(),point);
				fieldFactory.create(luceneFieldId,
									indexableField.getValue());
			}
		} else {
			// Create ONLY one field with the id = [fieldId]
			IndexDocumentFieldID luceneFieldId = IndexDocumentFieldID.fieldIdOf(indexableField.getFieldId());
			fieldFactory.create(luceneFieldId,
								indexableField.getValue());
		}
	}
